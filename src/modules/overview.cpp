...
void animate_tile_rect(OverviewState &state, const std::string &address,
                       const Rect &from, const Rect &to) {
    auto anim = [&](int component, float from_v, float to_v,
                    float Rect::*field) {
        state.base.animations.animate(
            from_v, to_v, kOverviewAnimFastMs, Easing::EaseOutCubic,
...
void rebuild_tiles(OverviewState &state, WaylandState &app, const GridLayout &g,
    ...
    for (auto it = state.tile_anim.begin(); it != state.tile_anim.end();) {
        bool live = std::any_of(
            ...
            [&](const OverviewWindowTile &t) { return t.address == it->first; });
...
void overview_toggle(OverviewState &state, WaylandState &app, bool by_widget) {
    ...
    state.base.animations.animate(
        ...
        kOverviewAnimFastMs, Easing::EaseOutCubic,
        [&state](float v) { state.slide_y = v; }, {}, kOverviewSlideOwner);
         ...
         [&overview, &state] {
             if (!overview.base.open) {
                 ...
                 if (target && (target->output.wl != overview.bound_output ||
                     ...
                     overview_retarget(overview, state.compositor, state.layer_shell,
        ...
        if (target) {
            ...
            if (active_in_page && state.slide_y == 0.0f) {
                ...
                if (!state.indicator_tracking ||
                    ...
                    state.base.animations.animate(
                        state.indicator_anim.x, cell.x, kOverviewAnimFastMs,
                        Easing::EaseOutCubic,
                        [&state](float v) { state.indicator_anim.x = v; }, {},
                        kOverviewIndicatorXOwner);
                    state.base.animations.animate(
                        state.indicator_anim.y, cell.y, kOverviewAnimFastMs,
                        Easing::EaseOutCubic,
                        [&state](float v) { state.indicator_anim.y = v; }, {},
                        kOverviewIndicatorYOwner);
                    state.indicator_target = cell;
                }
