...
- `gl.h`+`.cpp`: Shader compile/link helpers with error logging.
...
- `expanse.h`+`.cpp`: Per-monitor background surface; defines `ExpanseColumn` (one column's static `Texture` or animated `media_service` playback + zero-copy `VideoTexture`, generation-guarded upload, fill-mode draw) and drives a `ExpanseColumn` vector synced from config, drawn shared by `penance` and blink ambient.
