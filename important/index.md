...
- `module.h`: `Module` interface, the per-surface overlay boundary; default no-op virtuals, unnamed params; `apply_config` hook fired for every overlay on a config change; `on_output_removed` hook so an overlay bound to an unplugged output tears its surface down.
...
- `module_registry.h`+`.cpp`: `build_app_modules`/`build_per_monitor_modules` composition root, one subclass per overlay/per-monitor surface; also the `lock_notify_output_*`/`lock_is_locked`/`lock_start` bridge so `app/` code reaches the lock module without a module include.
...
- `settings_config.h`: Settings panel layout, opacity, animation, group, widget, popup, spinner constants, `SettingsFieldId` enum, `SettingsTabDef` type, and the six nav-rail tab labels (the icon-paired table itself is assembled in `settings.cpp`).
...
- `visualizer_config.h`: Audio visualizer surface-derived square render canvas (`0.75 * min(w,h)`), `11 kHz` stereo capture, CPU FFT, GLava GPU-transform constants, bar-visualizer geometry (`kVisualizerBar*`), plus `VisualizerParams` runtime knobs (`visualizer_shape` (`VisualizerShape` `Bar`/`Sphere`, default `Bar`), fps, particle thin/size, fractal complexity, glow directions/quality) and their clamp ranges (the shared `0.7` black backdrop moved to `render/palette.h`'s `window_backdrop`).
...
- `text.h`+`.cpp`: Cairo+Pango text rasterization, fixed body/small/large font sizes, `rasterize_text_px` for one-off pixel sizes (lock-screen clock), `kokusei_text_advance` fixed monospace cell width, and `make_text_texture` string-to-`Texture`.
...
- `overlay_panel.h`+`.cpp`: Shared full-screen on-demand overlay surface, position-lock-on-toggle recipe, `PanelHeightReveal` live-height roll-down/collapse helper, and `overlay_panel_release_output` (drop surface + clear `bound_output` when that output is unplugged).
...
- `layer_surface.h`+`.cpp`: Shared layer-shell surface creation helper, dedupes anchor/margin/listener setup; `destroy_layer_surface` takes an optional `FrameClock*` and drops its pending frame callback before tearing the surface down.
...
- `text_field.h`+`.cpp`: Shared single-line editable text buffer core (key handling, UTF-8-safe backspace, compose/IME preedit) plus every shell input's shared animations: `text_field_idle_toggle`/`draw_text_field_caret` caret idle, `TextFieldTypeAnim`+`text_field_type_anim_sync`/`_settle`/`_clear` per-character `EaseOutBack` pop, `TextFieldRowSlide`+`text_field_row_slide`/`_reset` `Behavior`-on-`x` whole-row slide for centered fields whose origin moves with length, and `draw_text_field_value` fixed-advance per-glyph value draw; `draw_text_field_preedit`.
...
- `notification_service.h`+`.cpp`: `org.freedesktop.Notifications` D-Bus server, `NotificationRecord` store (`Notify` add/replace, `CloseNotification` erase), and the expiry sweep; consumed by the `notification` renderer and the `lock` screen's notification dock.
...
- `logout.h`+`.cpp`: Logout ring state, rasterizer, burst entry/exit choreography (logo pop, hold, eight fast overshooting `thunder_burst` slashes along the `{8/3}` star path, hold, a `thunder_shock` shockwave plus one big left-to-right upward-slanted finishing slash that push the buttons out together; close runs the same slashes, hold, and shockwave/finishing slash but the buttons spread further out while fading and the logo just fades, no zoom), config-driven static/animated `AnimatedImage` centre logo, input dispatch, and paint. Also holds the two single-pass `GLES2` fragment effects reached through `Renderer::draw_custom`, each with its own lazily-compiled program, bypassing the `Node`/`Scene` graph: `thunder_burst_draw` (`kThunderBurstFs`) a forked-lightning bolt scoped to its segment bbox for the eight star-path slashes and the finishing slash; `thunder_shock_draw` (`kThunderShockFs`) a radial shockwave ring for the button-push burst.
- `dashboard.h`+`.cpp`: Dashboard singleton state, fixed top-right overlay, IPC/widget-triggered open wiring, clamped/scrollable card layout, brightness card driving `brightness_set`, `AnimatedImage` profile avatar freed while closed.
...
- `wallpaper.h`+`.cpp`: Per-monitor background surface; defines `WallpaperColumn` (one column's static `Texture` or animated `media_service` playback + zero-copy `VideoTexture`, generation-guarded upload, `weak_ptr` lifetime token on the deferred decode closures, fill-mode draw) and drives a `WallpaperColumn` vector synced from config, drawn shared by `lock` and idle ambient. Static columns cross-transition on image change: `wallpaper_column_upload_pending` keeps the old `Texture` as `tex_prev`, and `wallpaper_paint` runs a wall-clock `EaseInOutCubic` `draw_custom` pass (one of the six `wallpaper/*.frag` shaders, fill-mode uv computed CPU-side) after `wp.scene.draw`. `WallpaperPerMonitorModule::tick` clears `WallpaperState::visible` and pauses column decode while a fullscreen overlay (`logout`/`overview`) covers the output; `wallpaper_paint` no-ops while `!visible` or `session_locked`, but decode keeps running under a lock so `lock` composites the live wallpaper.
...
- `settings.h`+`.cpp`: Settings panel core, hosts per-tab modules, responsive nav rail, owns shared toggle-row widgets and `draw_profile_block`. Chrome and rail draw into `state.scene`; the active tab's content draws into a second `state.tab_scene` drawn at `base.opacity * tab_alpha`, so a tab switch runs a chained fade-out/swap/fade-in on `base.animations` without fading the rail.
...
- `visualizer.h`+`.cpp`: Audio visualizer overlay window; render thread runs either `SphereVisualizer` (ported `ncs`/WayVes Perlin-noise blob, `accent`-tinted over a `0.7` black backdrop, plus `glow` post) or `BarVisualizer` (log-frequency bars), selected by `VisualizerParams::visualizer_shape`; fed by own `11 kHz` stereo PipeWire capture, CPU FFT, and a GLava GPU transform chain; dedicated render thread and share-context `EGLContext`; render thread self-paces to `VisualizerParams::fps` for the sphere and a fixed `kVisualizerBarFps` for the bars, and reads live knobs via `visualizer_apply_params` (`Module::apply_config`).
- `lock.h`+`.cpp`: `ext-session-lock-v1` session lock; one lock surface per `wl_output`, `PAM` auth on a worker thread, `caelestia`-style fixed-ratio card with a three-column layout (battery/fetch/media, center clock+date+avatar+pill, resources/notifications) drawn from `mpris`/`system_stats`/`cpu_temp`/`gpu_temp`/`upower`/`notification_service`, entrance/exit spin-expand choreography.
...
- `sphere_visualizer.h`+`.cpp`: `SphereVisualizer`, render-thread `sphere-1` (atomic-image particle accumulation) -> `sphere-2` (blob resolve) -> `glow` post, at a square canvas of `visualizer_canvas_size(surfaceW, surfaceH)` rebuilt (atomic texture + three `RGBA8` FBOs) whenever the surface size changes, with `glMemoryBarrier` between stages, `u_fade`/`u_accent` plus the `VisualizerParams` knob uniforms (`particleThin`/`u_particleSize`/`u_complexity`/`u_glowDirections`/`u_glowQuality`), a full-window `glClear` to `window_backdrop` (alpha `* fade`), and a centered premultiplied textured-quad present of the canvas over that backdrop.
- `bar_visualizer.h`+`.cpp`: `BarVisualizer`, render-thread single-pass `kBarFs` fragment shader (`gl_FragCoord`-based, scissored to the bar band) sampling the `audio_stages` `smooth_l`/`smooth_r` textures; accent-tinted bottom-anchored rounded bars, count derived from window width, `kVisualizerBarMinHeight` floor when silent, over the same `window_backdrop` clear.
- `visualizer_shaders.h`+`.cpp`: `sphere1_fs`/`sphere2_fs`/`glow_fs`, concatenating the `assets/shaders/visualizer/sphere/*.glsl` fragments (includes inlined, `#expand` hand-expanded, `common.glsl` and the `*_head.glsl` prefixes shared) into the flattened `sphere`/`glow` shader at runtime; every complete `visualizer` shader is a file under `assets/shaders/visualizer/` (`sphere/` and `bar/` for the shape-specific ones) loaded directly by its consumer.

## src/modules/lock

- `layout.h`+`.cpp`: Pure lock-panel geometry math (fixed-ratio card size, three-column split, equal-height side-card split, center scale, content-stack height, fetch colour-box count, dot row); no `EGL`, linked into the test binary.
- `pam_authenticator.h`+`.cpp`: `pam_start_confdir`-based password check for the current user against the shipped `kokusei` `PAM` service, with a `login` fallback; run off the poll thread by the lock module.
...
- `visualizer_tab.h`+`.cpp`: Per-tab settings UI and commit logic; `Bar`/`Sphere` visualizer-shape selector row, then a number-field row per `VisualizerParams` knob with per-row reset shown only when `Sphere` is selected.
...
- `shaders/**`: Every `#version 320 es` `GLES` shader the shell compiles - `renderer/` (shared `Renderer` quad `vs` + rect/tex/rrect/rounded-tex/video `fs`), `logout/` (two `thunder` `fs`), `visualizer/` (shared `vs`/audio-pass `fs`, `bar/` and `sphere/` for the shape-specific shaders, `sphere/*.glsl` fragments assembled at runtime), `wallpaper/` (six cross-transition `fs` on `renderer/quad.vert`: `fade`/`wipe`/`disc`/`stripes`/`zoom`/`honeycomb`); installed as a subdir by meson, `NOTICE` records the `lygia`/`GLava`/`noctalia` third-party parts.
