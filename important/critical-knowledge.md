...
- **`precision mediump float` in a fragment shader is real on NVIDIA (range only `±2^14 = 16384`), fake on Mesa (promoted to highp).** `starward`'s thunder shaders rendered as a filled rectangle on NVIDIA because `length(ab)` / `dot(ab,ab)` on a ~900 px bolt vector overflowed mediump to `+inf`, so `dir = ab/len` collapsed to `0` and every fragment computed the same value. `rrect` uses mediump too but only `length()`s ~50 px SDF vectors, so it never overflowed. Declare `precision highp float` unconditionally for any fragment shader that touches pixel-scale magnitudes (`length`, `dot`, `d*d`, hashes); `#ifdef GL_FRAGMENT_PRECISION_HIGH` is NOT reliably defined on NVIDIA GLES2.
