...
- **`pow(0.0, 0.0)` is spec-undefined and diverges: NVIDIA returns `1.0`, Mesa Intel returns `NaN` (`exp2(0 * log2(0))` = `exp2(NaN)`).** `visualizer`'s `glowLightVal` did `pow(length(FragColor), glow.brightnessOffset)` with both `0` in every blob-free glow pixel, so the whole glow FBO was `NaN` on Mesa and the blitted canvas rendered as a bright square instead of the `0.7` black scrim. Guard any `pow` base that can reach `0` with `max(base, 1e-4)`.
...
- **A single-pass module shader can ride `Renderer::draw_custom` instead of owning FBOs.** `logout`'s `thunder_burst`/`thunder_shock` compile lazily, reuse `kRendererQuadVs` and the shared `quad_vbo_`, have no uniform-bounded loop, and each draw covers only its effect's bounding box, not the output.
...
- **Never drive GL for a surface the compositor has stopped compositing.** `wallpaper_paint` no-ops while its output is covered or the session is locked; an animated wallpaper behind the lock keeps decoding and is composited into the visible `lock` lock surface, never swapped on its own hidden background surface.
...
- **`visualizer_shaders::kFullscreenVs` emits only `gl_Position`; a fragment shader paired with it must use `gl_FragCoord`, not a `vUv` varying.** `kBarFs` first declared `in vec2 vUv` and failed to link (`no matching output`), so `bar_ok` stayed false and the visualizer just cleared; only `kPresentVs` provides `vUv`.
- **`visualizer`'s `fps` knob throttles only the sphere; the render thread paces the bars at a fixed `kVisualizerBarFps`.** The knob defaults low on Intel to tame the heavy `ncs` blob; letting the cheap bar shader inherit `15 fps` made it visibly choppy.
...
- **Every `ncs.glsl` / `ncs` default constant must be kept verbatim; the shader is a tuned whole and "roughly similar" tweaks all read wrong.** Reverted divergences: `sphere.radius = 0.5 * min(...)` (rectangle-with-hole) and a `baseForm.scale` override (should stay `ncs`'s `2.0`). The `accent`-hued blob/glow is a deliberate kokusei override, fed as the `u_accent` uniform so every shader-literal constant stays verbatim; the `0.7` black backdrop is painted host-side (see below), not in the shader. The per-device knobs (`particleThin`, `particle.size`, `fractalField.complexity`, `glow.directions`, `glow.quality`, render `fps`) are also uniforms now, defaulted in `VisualizerParams` to the exact `ncs` constants — "keep verbatim" means the defaults equal the verbatim values.
- **`visualizer`'s blob canvas is a square `visualizer_canvas_size(surfaceW, surfaceH)` (`0.75 * min(w,h)`), not a fixed size, rebuilt (atomic texture + FBOs) whenever the surface resizes, then `glBlitFramebuffer`d centered onto the window.** This ports `ncs`'s `applyResize` (`ncs` uses `min/2`; kokusei uses `0.75` so the oversized ring clips closer to the window edge and the `0.7` backdrop margin stays thin). The blob geometry (`sphere.radius = .7236 * resolution.x`, `bassMultiplier`, `displacements`) is pixel-absolute against the canvas via the `resolution` uniform, which now carries the canvas size; the canvas must stay **square** or `sphereCoords()` yields an ellipsoid — squareness is the invariant, not a fixed number. An earlier whole-surface textured-quad present stuttered the visualizer and was reverted to a blit; the present is now a quad again but scoped to just the canvas rect (see the backdrop entry below), ~35% of the window's pixels, so the fill cost that caused the stutter does not apply.
- **`ncs`'s grid-step divisor is `0` whenever `baseForm.numParticles.z == 1` (the `visualizer` default), and only Intel turns that into a GPU hang.** `int(Inf)` clamps high on Mesa/NVIDIA (loop exits at once) but is `0` on Intel, then `-1` after `spaces -= 1`, so the `for i += spaces.z` loop runs backwards forever. The flattened `kNcs1Main` must carry `ncs`'s guard: `max(denom, vec3(0.0001))`, `max(spaces.x/y, 1)`, and the `numParticles.z <= 1.0 ? int(zSize)+1 : max(spaces.z, 1)` branch.
- **`ncs` never `glClear`s and its glow pass is premultiplied, transparent where the blob is absent; `visualizer` gives the whole window a `0.7` black backdrop `ncs` does not, painted host-side so the ported shader stays verbatim.** The `glow` shader output is left premultiplied-transparent (no `u_backdrop`); the render thread `glClear`s the whole default framebuffer to `window_backdrop` with alpha `* fade`, then presents `glow_tex_` as a premultiplied textured quad (`GL_ONE, GL_ONE_MINUS_SRC_ALPHA`) over the centered canvas rect. `u_fade` now fades only the blob; the backdrop fades via the clear alpha. Backdrop compositing was in the `glow` shader before (`FragColor += (1 - FragColor.w) * u_backdrop`) and broke on Mesa when `pow(0,0)` made that pixel `NaN` — host-side clear is immune.
- **`visualizer`'s render thread must present every frame and start its fade on the first presented frame, never gate either on a frame-time budget.** An earlier build dropped the `eglSwapBuffers` for any frame over `60 ms` and only started the fade once a frame came in under `30 ms`. On Intel the untuned `ncs` blob is ~110 ms/frame, so the `xdg_toplevel` never committed a buffer (no window at all); tuning the knobs under `60 ms` mapped the window but frames stayed over `30 ms`, so `fade` was pinned at `0` and the canvas stayed transparent. The blob being heavy is what the per-device knobs are for; the render loop just runs at whatever rate the hardware gives.
- **`ncs`'s `time` uniform is a frame counter paced to `fps` (default 60), and its GLava decay constants are per-frame at that rate.** `ncs` `nanosleep`s its render loop to `fps`; on a high-refresh display an un-paced port scrolls the `flows` noise and decays the spectrum too fast and the blob reads as over-reactive. `fps` is a `VisualizerParams` knob; the render thread's frame step AND the gravity-decay divisor (`VisualizerAudioStages::run`'s `fps` arg) both use the live value so lowering it stays wall-clock-consistent like `ncs`.
