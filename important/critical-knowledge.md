...
- **Modules are named after Keqing, not their function; IPC verbs and code identifiers moved too.** See `index.md`'s `src/modules` for the name↔function map; `starward` and `rain` are the only non-Keqing names kept.
