...
- **`resonance_shaders::kFullscreenVs` emits only `gl_Position`; a fragment shader paired with it must use `gl_FragCoord`, not a `vUv` varying.** `kBarFs` first declared `in vec2 vUv` and failed to link (`no matching output`), so `bar_ok` stayed false and the visualizer just cleared; only `kPresentVs` provides `vUv`.
- **`resonance`'s `fps` knob throttles only the sphere; the render thread paces the bars at a fixed `kResonanceBarFps`.** The knob defaults low on Intel to tame the heavy `ncs` blob; letting the cheap bar shader inherit `15 fps` made it visibly choppy.
...
- **The shared `load_image_decode` / `decode_svg` renders SVGs into a square viewport only.** Aspect-correct SVG rasterization (e.g. `stiletto_grid`'s tall sprite) needs a direct `librsvg` render into a sized `cairo` surface.
