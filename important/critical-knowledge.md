...
- **A ported menu's outer inset is `borderWidth + innerMargins`, distinct from its inter-element spacing.** `launcher` needed a separate `kLauncherMenuPad` (`14`) beside `kLauncherPad` (`10`) to reproduce the height.
...
- **A two-line row needs more than `keqing-shell`'s `entryHeight 40` and a positive inter-line gap.** `launcher` rows are `44` with a slightly negative gap so fixed-metric title/subtitle boxes overlap in their whitespace.
...
- **`penance`'s lock card is a fixed `heightMult`/`ratio` rectangle sized from output height, not from content.** The `caelestia` layout wants a large stable card the three columns fill, unlike the `keqing-shell` port.
